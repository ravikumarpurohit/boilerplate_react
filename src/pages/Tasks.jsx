...
const Tasks = () => {
  ...
  //   e.preventDefault();
  //   if (openMode === "add") {
  //     setTaskFromData({ ...taskFromData, businessId: businessId });
  //     const result = await createProduct(token, taskFromData);
  //     if (!result.error) {
  //       handleGetTaskList();
  //       handlePopupClose();
  //     } else {
  ...
  //   setMessage("");
  //   setTaskFromData(initProductData);
  //   setPopupOpen(false);
  // };
  return (
    <>
        ...
        {taskList.length > 0 &&
          taskList.map((task) => (
            <div className=" grid grid-cols-8 bg-white" key={task._id}>
              <div className=" py-2 pl-8 pr-3 text-lg font-medium text-[#343434] col-span-2 overflow-clip text-ellipsis">
                {task.taskTitle}
              </div>
